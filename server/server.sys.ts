...
module NodeServer {
    class HttpService {
        ...
        constructor(public port?: number, public cors?: boolean) {
            if (!(this.port) || typeof (this.port) != 'number') this.port = 48524;
        }
        ...
        private handler = (request: http.ServerRequest, response: http.ServerResponse) => {
            ...
            if (this.cors) {
                //if (request.method.toUpperCase() == 'OPTIONS') {
                    ...
                    //console.log('response set header: ', response.getHeader('Access-Control-Allow-Origin'));
                //}
            }
            tryNext();
        };
    ...
    class RouteMiddleware implements IMiddleware {
        constructor(public route: RegExp, public handler: (request: http.ServerRequest, response: http.ServerResponse, next: () => void) => void) {
    ...
    class FileMiddleware implements IMiddleware {
        ...
        */
        ...
        handler = (request: http.ServerRequest, response: http.ServerResponse, next: () => void) => {
            ...
            if (/\/$/.test(filename)) filename += this.default;
...
<div>Simple Node Service</div>\
                            ...
                            default:
                                    fs.readFile(scriptFile, (err: NodeJS.ErrnoException, data: Buffer) => {
                                        ...
                                        try {
                                            ...
                                            precode = precode.replace(/(^|;)(\s*(let|var|const)\s+(\w+)\s*=\s*require\s*\(\s*\'errisy-cgi\'\)\s*;)/, (capture: string, ...args: any[]) => {
                                                varCGI = args[3];
                                                return args[0];
                                            });
                                            let code = '(function (){\ntry{\n'
                                                + precode.replace(/require\s*\(\s*[\'"]\s*\[\s*(\w+)\s*\]\s*[\'"]\s*\)/ig, (capture: string, ...args: any[]) => {
                                                ...
                                                try {
                                                    ...
                                                    switch (memberType) {
                                                        ...
                                                        case '-':
                                                            ...
                                                            let invokePromise: Promise<any> = $Object[memberName].apply($Object, paramaters);
    ...
    if (process.send) {
        ...
        process.on('message', (data: string) => {
            let options: { Port: number, CORS: boolean } = <any>JSON.parse(data);
            let assignedPort = options.Port;
            let parsedPort = 1018;
            if (assignedPort) parsedPort = Number(assignedPort);
